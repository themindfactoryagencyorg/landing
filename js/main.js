...
function panelUI() {
  ...
  if (selectedMind === "billing")
    return `<div class="ui-toolbar"><span>${tr("Del pedido a la factura", "From order to invoice")}</span><span class="ui-tag">#0248</span></div><div class="metric">01 → 01</div><p class="metric-label">${tr("Un pedido. Una factura vinculada.", "One order. One linked invoice.")}</p><div class="invoice-row"><span>${tr("Datos del cliente", "Customer details")}</span><small>${tr("Conectados", "Connected")}</small><b><svg class="icon icon-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="m5 12 4 4L19 6"/></svg></b></div><div class="invoice-row"><span>${tr("Líneas del pedido", "Order items")}</span><small>24 u.</small><b><svg class="icon icon-check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="m5 12 4 4L19 6"/></svg></b></div><div class="invoice-row"><span>${tr("Estado", "Status")}</span><small>${tr("Lista para revisar", "Ready to review")}</small><b><svg class="icon icon-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 19 19 5M5 5h14v14"/></svg></b></div>`;
...
function renderPanel(animate = false) {
  ...
  content.innerHTML = `<div class="panel-top"><span>MindHub / ${text(names[selectedMind])}</span><span aria-hidden="true"><svg class="icon icon-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 19 19 5M5 5h14v14"/></svg></span></div><h3 class="panel-title">${text(titles[selectedMind])}</h3><p class="panel-intro">${text(intros[selectedMind])}</p><div class="panel-ui">${panelUI()}</div><div class="panel-note"><span aria-hidden="true"><svg class="icon icon-star" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 2v20M2 12h20M5 5l14 14M5 19 19 5"/></svg></span><span>${text(notes[selectedMind])}</span></div>`;
...
async function simulateSale() {
  ...
  button.innerHTML =
    tr("Repetir demostración", "Replay demonstration") +
    ` <span aria-hidden="true"><svg class="icon icon-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M5 19 19 5M5 5h14v14"/></svg></span>`;
}
function initTabs() {
